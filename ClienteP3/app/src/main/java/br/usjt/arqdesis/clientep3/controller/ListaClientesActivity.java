...
import br.usjt.arqdesis.clientep3.util.APIClient;
...
public class ListaClientesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(ListaClientesActivity.this,"Carregando...",
                Toast.LENGTH_SHORT).show();
        if(chave !=null && chave.length()>0){
            new APIClient().getRestService().
                    getAllPoetaLikeName(chave,callBackCliente);
        } else{
    ...
    private void configurarCallbackCliente() {
        callBackCliente = new Callback<List<Cliente>>() {
            @Override
            public void success(List<Cliente> lista, Response
                    response) {
